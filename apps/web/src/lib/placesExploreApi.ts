import type { ExplorePlaceCategory } from "../constants/placeCategories";
import { API_BASE } from "../constants/syncConfig";
...
export async function fetchExplorePlaces(
  ...
  const res = await fetch(`${apiBase()}/pois/explore?${params.toString()}`);
  if (!res.ok) {
